# CLAUDE.md · Claude Code — CTF Engagement Framework
...
You are the **orchestrator**. Each phase has a subagent in `.claude/agents/` (`htb-recon`, `htb-web`, `htb-ad`, `htb-privesc`, `htb-report`, `htb-learn`) — delegate to it and
route its findings to the next phase. Consult skills in `.claude/skills/` (the Skill tool) by phase.
Subagents run in isolation and cannot spawn other subagents. On independent targets (multiple
vhosts/hosts), run subagents in **parallel**.
